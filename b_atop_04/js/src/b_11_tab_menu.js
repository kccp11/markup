...
/*   $.getJSON("../json/menu.json",function(data){
  ...
  $.ajax({
    ...
    // title에 따른 메뉴 구성 : 함수 호출은 -> tabMenuFn(순서);
    ...
    var tabTitile = tabArea.find('.tab_title')
    var tabContent = tabArea.find('.tab_content');

    // tabTitle 내부에 탭메뉴 제목부 구성 tabTitleCheck를 이용
    tabTitile.html('<ul></ul>')
    var tabTUl = tabTitile.children('ul');
...
/*     $.each(tabTitleCheck, function(index,data){
    ...
    tabBtn.on('click',function(e){
      ...
      //tabMenuFn(n);
      //tabConMenu.text(JSON.stringify(tabMenuSet));
      // tabvMenuFn을 통해 찾아낸 타입구성을 재설계하여 배치할 수 있는 구성을 함수로 만들어서 호출
      tabMenuSetFn(n)
    });

    //객체형식을 문자로 변환 : JSON.stringifly(객체)
