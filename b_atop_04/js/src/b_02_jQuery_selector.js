...
(function($){
  ...
  //=========================================================
  //선택자1 : $('css선택자)
  //css표현 : 선택자.css({'속성명':'값', '속성명':'값', '속성몇':'값'});
  $('.ex_wrap').css({
    'backgroundColor' : '#fff',
  ...
  var box = $('#box');
  box.css({ 'backgroundColor': '#dfd', 'padding':20+10 + 'px'});
  ...
  boxList.css({'border': '1px solid #555','margin':10 + 'px auto'});
  
  //==============================================
  //선택자에 순서를 가르키는 것 -> eq(순번) : 순번은 첫번째가 0 두번쨰가 1
  ...
  boxUl_01_li.eq(0).next().css({'backgroundColor' : '#dd7'});//next 다음요소
  boxUl_01_li.eq(0).nextAll().css({'color' : '#f06'});
  boxUl_01_li.eq(4).css({'fontSize' : '1.2rem'});
  boxUl_01_li.eq(-2).prev().css({'textIndent' : '30px'});//prev 이전요소
  ...
  boxOl_li.eq(5).siblings().css({'fontWeight':700, 'color' : '#f06'});//siblings 요소를 제외한 나머지
